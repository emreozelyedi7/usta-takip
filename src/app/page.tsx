    ...
    <div className="max-w-xl mx-auto bg-slate-50 min-h-screen text-slate-900 overflow-x-hidden flex flex-col pb-32">
        ...
        {aktifSekme === 'teklifler' && (
          <div className="space-y-4">
            <div className="flex gap-2">
              <select value={seciliYil} onChange={(e) => setSeciliYil(Number(e.target.value))} className="flex-1 bg-slate-100 rounded-xl px-3 py-3 text-base font-bold outline-none border-none">
                {yillar.map(y => <option key={y} value={y}>{y}</option>)}
              </select>
              <select value={seciliAy} onChange={(e) => setSeciliAy(Number(e.target.value))} className="flex-1 bg-slate-100 rounded-xl px-3 py-3 text-base font-bold outline-none border-none">
            ...
            <div className="flex overflow-x-auto gap-2 pb-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
              {Array.from({ length: new Date(seciliYil, seciliAy + 1, 0).getDate() }, (_, i) => i + 1).map(gun => (
                <button key={gun} onClick={() => setSeciliGun(gun)} className={`flex-shrink-0 w-10 h-12 rounded-xl flex flex-col items-center justify-center transition-all ${seciliGun === gun ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-50 text-slate-400'}`}>
      ...
      <div className="p-3 space-y-4 flex-1">
        {teklifler.map(t => {
          ...
          return (
            <div key={t.id} onClick={() => aktifSekme === 'uretim' && setSelectedJob(t)} className="bg-white p-5 rounded-[2rem] shadow-sm border border-slate-100 active:scale-[0.98] transition-all">
                <div className="flex justify-between items-start mb-2">
                    <div className="flex-1 pr-2">
                        <h2 className="font-bold text-[18px] leading-tight text-slate-800">{t.musteri}</h2>
                        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{t.is_modeli} — {t.ilce}</p>
                        <span className="text-[9px] bg-slate-50 px-2 py-1 rounded-md font-bold text-slate-400 uppercase mt-2 inline-block border border-slate-100">{t.kaynak}</span>
                    </div>
                    <div className="text-right text-base font-black text-slate-900">{t.fiyat}</div>
                </div>

                {aktifSekme === 'teklifler' && t.aciklama && (
                  <div className="mt-2 mb-2 bg-slate-50 p-3 rounded-xl border-l-4 border-slate-200">
                    <p className="text-[11px] text-slate-500 font-medium italic leading-relaxed">{t.aciklama}</p>
                  </div>
                )}
                ...
                {aktifSekme === 'uretim' ? (
                  ...
                  <select 
                    ...
                    className={`w-full mt-3 text-sm font-black uppercase py-3.5 px-4 rounded-2xl border-none outline-none transition-all shadow-sm ${
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-100 px-12 py-6 flex justify-around items-center z-50 rounded-t-[2.5rem] shadow-2xl">
        <button onClick={() => setAktifSekme('teklifler')} className={`flex flex-col items-center gap-1 transition-all ${aktifSekme === 'teklifler' ? 'text-blue-600' : 'text-slate-300'}`}>
            <span className="text-[11px] font-black uppercase tracking-widest text-center">TEKLİFLER</span>
            {aktifSekme === 'teklifler' && <div className="w-1.5 h-1.5 bg-blue-600 rounded-full"></div>}
        </button>
        <button onClick={() => setAktifSekme('uretim')} className={`flex flex-col items-center gap-1 transition-all ${aktifSekme === 'uretim' ? 'text-blue-600' : 'text-slate-300'}`}>
            <span className="text-[11px] font-black uppercase tracking-widest text-center">ÜRETİM</span>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm p-8 rounded-[3rem] shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={isKaydet} className="space-y-4">
              <input required className="w-full bg-slate-100 p-4 rounded-xl text-base font-bold outline-none border-none" placeholder="Müşteri Adı" onChange={(e) => setYeniIs({...yeniIs, musteri: e.target.value})} />
              <div className="grid grid-cols-2 gap-2">
                <select className="bg-slate-100 p-4 rounded-xl text-base font-bold border-none outline-none" onChange={(e) => setYeniIs({...yeniIs, isModeli: e.target.value})}>
                ...
                <select className="bg-slate-100 p-4 rounded-xl text-base font-bold border-none outline-none" onChange={(e) => setYeniIs({...yeniIs, kaynak: e.target.value})}>
                    <option value="Instagram">Instagram</option><option value="Referans">Referans</option><option value="Web">Web</option>
                </select>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <input required className="w-full bg-slate-100 p-4 rounded-xl text-base font-bold outline-none" placeholder="Fiyat" type="number" onChange={(e) => setYeniIs({...yeniIs, fiyat: e.target.value})} />
                <input required className="w-full bg-slate-100 p-4 rounded-xl text-base font-bold outline-none" placeholder="İlçe" onChange={(e) => setYeniIs({...yeniIs, ilce: e.target.value})} />
              </div>
              <textarea className="w-full bg-slate-100 p-4 rounded-xl text-base font-bold h-24 resize-none outline-none" placeholder="Notlar..." onChange={(e) => setYeniIs({...yeniIs, aciklama: e.target.value})}></textarea>
              <button type="submit" className="w-full bg-blue-600 text-white p-5 rounded-2xl font-black text-base uppercase shadow-xl mt-2 active:scale-95">İŞİ KAYDET</button>
      ...
      {selectedJob && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-end justify-center">
          <div className="bg-white w-full max-w-md p-8 rounded-t-[2.5rem] shadow-2xl flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            ...
            <textarea value={selectedJob.aciklama || ''} onChange={(e) => aciklamaGuncelle(selectedJob.id, e.target.value)} className="w-full bg-slate-50 p-4 rounded-2xl text-base font-medium h-24 border-none outline-none" placeholder="Üretim notları..." />
            <div className="grid grid-cols-2 gap-3 mb-4">
              {[ { key: 'cizim', label: 'Çizim' }, { key: 'camSiparisi', label: 'Cam Sip.' }, { key: 'profil', label: 'Profil' }, { key: 'camGeldi', label: 'Cam Geldi' }, { key: 'atolye', label: 'Atölye' }, { key: 'teslim', label: 'Teslim' } ].map((item) => (
                <button key={item.key} onClick={() => uretimAsamasiGuncelle(selectedJob.id, item.key)} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all ${selectedJob.uretim?.[item.key] ? 'bg-emerald-500 border-emerald-500 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-400'}`}>
                    <span className="text-[10px] font-black uppercase">{item.label}</span>
                    <div className="text-xs">{selectedJob.uretim?.[item.key] ? '✓' : '○'}</div>
                </button>
              ))}
            </div>
            <button onClick={() => setSelectedJob(null)} className="w-full bg-slate-900 text-white p-5 rounded-2xl font-black text-sm uppercase">KAPAT</button>
          </div>
        </div>
      )}
    </div>
