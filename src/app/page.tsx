...
export default function Home() {
  ...
  const [selectedJob, setSelectedJob] = useState<any>(null); // Detay paneli için
    ...
    <div className="max-w-xl mx-auto bg-slate-50 min-h-screen text-slate-900 overflow-x-hidden flex flex-col pb-32">
          ...
          return (
            <div key={t.id} onClick={() => aktifSekme === 'uretim' && setSelectedJob(t)} className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 active:scale-[0.98] transition-all">
                ...
                {aktifSekme === 'uretim' ? (
                  ...
                  <select value={t.durum} onChange={(e) => durumGuncelle(t.id, e.target.value)} className="w-full mt-4 text-[10px] font-black uppercase py-3 px-4 rounded-2xl border-none outline-none bg-slate-50 text-slate-500">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-sm p-8 rounded-[3rem] shadow-2xl">
            <h3 className="text-xl font-black text-slate-900 mb-6 text-center uppercase">YENİ TEKLİF</h3>
            <form onSubmit={isKaydet} className="space-y-3 pb-4">
