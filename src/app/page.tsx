...
export default function Home() {
  ...
  return (
    <div className="max-w-xl mx-auto bg-slate-50 min-h-screen text-slate-900 overflow-x-hidden flex flex-col pb-28">
      ...
      {/* LİSTE ALANI - SIKILAŞTIRILDI */}
      <div className="p-3 space-y-3 flex-1">
        {teklifler.map(t => {
          const yuzde = ilerlemeHesapla(t.uretim);
          return (
            <div key={t.id} onClick={() => aktifSekme === 'uretim' && setSelectedJob(t)} className="bg-white p-4 rounded-[1.2rem] shadow-sm border border-slate-100 active:scale-[0.98] transition-all">
                <div className="flex justify-between items-start mb-1">
                    <div className="flex-1 pr-2">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className="text-[8px] bg-slate-50 px-1.5 py-0.5 rounded font-bold text-slate-400 uppercase border border-slate-100">{t.kaynak}</span>
                        </div>
                    </div>
                ...
                {aktifSekme === 'uretim' ? (
                  ...
                  <select 
                    ...
                    // py-3.5 İPTAL EDİLDİ, py-1.5 YAPILDI. TEXT-BASE KORUNDU.
                    className={`w-full mt-2 text-base font-black uppercase py-1.5 px-3 rounded-lg border-none outline-none transition-all shadow-sm leading-tight h-10 ${
      ...
      {/* ALT NAVİGASYON */}
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-100 px-12 py-4 flex justify-around items-center z-50 rounded-t-[1.5rem] shadow-2xl">
      ...
      {/* YENİ KAYIT MODALI - SIKILAŞTIRILDI */}
      {isModalOpen && (
      ...
      {/* ÜRETİM DETAY PANELİ - SIKILAŞTIRILDI */}
      {selectedJob && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-end justify-center">
          <div className="bg-white w-full max-w-md p-6 rounded-t-[2rem] shadow-2xl flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            ...
            <textarea value={selectedJob.aciklama || ''} onChange={(e) => aciklamaGuncelle(selectedJob.id, e.target.value)} className="w-full bg-slate-50 p-3 rounded-xl text-base font-medium h-20 border-none outline-none" placeholder="Üretim notları..." />
