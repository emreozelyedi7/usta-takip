...
export default function Home() {
  ...
  return (
    <div className="max-w-xl mx-auto bg-slate-50 min-h-screen text-slate-900 overflow-x-hidden flex flex-col pb-32">
        ...
        {aktifSekme === 'teklifler' && (
          <div className="space-y-3">
            <div className="flex gap-2">
              <select value={seciliYil} onChange={(e) => setSeciliYil(Number(e.target.value))} className="flex-1 bg-slate-50 rounded-xl px-3 py-2 text-xs font-bold outline-none border-none">
                {yillar.map(y => <option key={y} value={y}>{y}</option>)}
              </select>
              <select value={seciliAy} onChange={(e) => setSeciliAy(Number(e.target.value))} className="flex-1 bg-slate-50 rounded-xl px-3 py-2 text-xs font-bold outline-none border-none">
            ...
            <div key={t.id} onClick={() => aktifSekme === 'uretim' && setSelectedJob(t)} className="bg-white p-4 rounded-[1.8rem] shadow-sm border border-slate-100 active:scale-[0.98] transition-all">
                ...
                {aktifSekme === 'uretim' ? (
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <select value={t.durum} onChange={(e) => durumGuncelle(t.id, e.target.value)} className={`flex-1 text-[9px] font-black uppercase py-2 px-3 rounded-xl border-none outline-none transition-all ${
                      t.durum === 'onaylandi' ? 'bg-emerald-50 text-emerald-600' : 
                      t.durum === 'iptal' ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-600'
                    }`}>
      ...
      {/* ÜRETİM DETAY PANELİ (Maddeler Burada) */}
      {selectedJob && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-end justify-center">
          <div className="bg-white w-full max-w-md p-8 rounded-t-[2.5rem] shadow-2xl animate-in slide-in-from-bottom duration-300">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-sm p-8 rounded-[2.5rem] shadow-2xl my-auto overflow-hidden">
            ...
            <form onSubmit={isKaydet} className="space-y-3">
              <input required className="w-full bg-slate-50 p-4 rounded-xl text-sm font-bold outline-none border border-transparent focus:border-blue-500 transition-all" placeholder="Müşteri Adı" onChange={(e) => setYeniIs({...yeniIs, musteri: e.target.value})} />
              
              <div className="grid grid-cols-2 gap-2">
                <select className="bg-slate-50 p-4 rounded-xl text-[10px] font-bold outline-none border-none" onChange={(e) => setYeniIs({...yeniIs, isModeli: e.target.value})}>
                ...
                <select className="bg-slate-50 p-4 rounded-xl text-[10px] font-bold outline-none border-none" onChange={(e) => setYeniIs({...yeniIs, kaynak: e.target.value})}>
              ...
              <textarea className="w-full bg-slate-50 p-4 rounded-xl text-sm font-bold outline-none h-20 resize-none" placeholder="Notlar..." onChange={(e) => setYeniIs({...yeniIs, aciklama: e.target.value})}></textarea>
