...
export default function Home() {
  ...
  // Açıklamayı güncellemek için yeni fonksiyon
    ...
    <div className="max-w-xl mx-auto bg-slate-50 min-h-screen text-slate-900 overflow-x-hidden flex flex-col pb-32">
      ...
      <div className="p-3 space-y-3 flex-1">
        {teklifler.map(t => {
          ...
          return (
            <div key={t.id} onClick={() => aktifSekme === 'uretim' && setSelectedJob(t)} className="bg-white p-4 rounded-[1.8rem] shadow-sm border border-slate-100 active:scale-[0.98] transition-all overflow-hidden">
                <div className="flex justify-between items-start mb-1">
                    ...
                    <div className="text-right text-sm font-black text-slate-900 whitespace-nowrap">{t.fiyat}</div>
                </div>

                {/* SADECE TEKLİFLERDE GÖSTERİLEN AÇIKLAMA */}
                {aktifSekme === 'teklifler' && t.aciklama && (
                  <div className="mt-1 mb-2 bg-slate-50/50 p-2 rounded-lg border-l-2 border-slate-200">
                ...
                {aktifSekme === 'uretim' ? (
                  ...
                  <select value={t.durum} onChange={(e) => durumGuncelle(t.id, e.target.value)} className="w-full mt-3 text-[9px] font-black uppercase py-2.5 px-3 rounded-xl border-none outline-none bg-slate-50 text-slate-500">
      ...
      {/* ÜRETİM DETAY PANELİ (Not Düzenleme Alanı Eklendi) */}
      {selectedJob && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-end justify-center">
          <div className="bg-white w-full max-w-md p-8 rounded-t-[2.5rem] shadow-2xl animate-in slide-in-from-bottom duration-300 flex flex-col gap-4 max-h-[90vh] overflow-y-auto">
            ...
            {/* ÜRETİM NOTLARI - BURADA DÜZENLENEBİLİR */}
            <div className="space-y-2">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Atölye & Üretim Notları:</span>
                <textarea 
                    value={selectedJob.aciklama || ''}
                    onChange={(e) => aciklamaGuncelle(selectedJob.id, e.target.value)}
                    placeholder="Üretim için not girin..."
                    className="w-full bg-slate-50 p-4 rounded-2xl text-xs font-medium text-slate-600 border border-slate-100 outline-none focus:border-blue-500 h-24 resize-none"
                />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button key={item.key} onClick={() => uretimAsamasiGuncelle(selectedJob.id, item.key)} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all ${selectedJob.uretim?.[item.key] ? 'bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-100' : 'bg-white border-slate-100 text-slate-400'}`}>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-sm p-8 rounded-[2.5rem] shadow-2xl my-auto">
            ...
            <form onSubmit={isKaydet} className="space-y-3">
              <input required className="w-full bg-slate-100 p-4 rounded-xl text-sm font-bold outline-none border border-transparent focus:border-blue-500" placeholder="Müşteri Adı" onChange={(e) => setYeniIs({...yeniIs, musteri: e.target.value})} />
              <div className="grid grid-cols-2 gap-2">
                <select className="bg-slate-100 p-4 rounded-xl text-[10px] font-bold outline-none border-none" onChange={(e) => setYeniIs({...yeniIs, isModeli: e.target.value})}>
                    <option value="Cam Balkon">Cam Balkon</option>
                    <option value="Giyotin">Giyotin</option>
                    <option value="Sürme">Sürme</option>
                    <option value="Teras">Teras</option>
                </select>
                <select className="bg-slate-100 p-4 rounded-xl text-[10px] font-bold outline-none border-none" onChange={(e) => setYeniIs({...yeniIs, kaynak: e.target.value})}>
              ...
              <textarea className="w-full bg-slate-100 p-4 rounded-xl text-sm font-bold outline-none h-20 resize-none" placeholder="İlk Teklif Notları..." onChange={(e) => setYeniIs({...yeniIs, aciklama: e.target.value})}></textarea>
