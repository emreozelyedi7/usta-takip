...
export default function Home() {
  ...
  const isKaydet = async (e: React.FormEvent) => {
    ...
    if (!error) { verileriGetir(); setIsModalOpen(false); setYeniIs({ musteri: "", isModeli: "Cam Balkon", adet: "1", ilce: "", fiyat: "", kaynak: "Instagram", aciklama: "" }); }
  };
  ...
  const uretimAsamasiGuncelle = async (teklifId: number, asama: string) => {
    ...
    if (asama === 'teslim' && guncelUretim.teslim === true) {
        ...
        delete guncelUretim.teslimatBitisTarihi;
    }
  ...
  return (
    <div className="max-w-xl mx-auto bg-slate-50 min-h-screen text-slate-900 overflow-x-hidden flex flex-col pb-28 relative" onTouchStart={(e)=>touchStartRef.current=e.targetTouches[0].clientX} onTouchEnd={(e)=>{if(!touchStartRef.current)return; if(touchStartRef.current-e.changedTouches[0].clientX < -120 && !isModalOpen && !selectedJob)setIsSidebarOpen(true); touchStartRef.current=null;}} style={{touchAction:'pan-y'}}>
        ...
        <>
          ...
          <div className="fixed top-0 left-0 bottom-0 w-3/4 max-w-xs bg-white z-[120] shadow-2xl animate-in slide-in-from-left duration-300 flex flex-col">
            ...
            <div className="flex-1 p-4 space-y-2 mt-2">
              {['teklifler','siparisler','uretim'].map((s)=>(<button key={s} onClick={()=>{setAktifSekme(s); setIsSidebarOpen(false);}} className={`w-full flex items-center gap-3 p-4 rounded-2xl font-black text-xs uppercase transition-all ${aktifSekme===s ? 'bg-blue-50 text-blue-600' : 'text-slate-500'}`}>{s==='teklifler'?'Teklif Arşivi':s==='siparisler'?'Siparişler':'Üretim Hattı'}</button>))}
            </div>
      ...
      <div className="bg-white px-4 pt-8 pb-3 border-b border-slate-100 shadow-sm sticky top-0 z-40">
        <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2">
                ...
                <h1 className="text-xl font-black uppercase">{aktifSekme==='teklifler'?'Teklif Arşivi':aktifSekme==='siparisler'?'Siparişler':'Üretim Hattı'}</h1>
            </div>
            <button onClick={() => setIsModalOpen(true)} className="bg-slate-900 text-white w-8 h-8 rounded-xl font-bold shadow-md">+</button>
        </div>
                ...
                <div onClick={() => aktifSekme !== 'teklifler' && setSelectedJob(t)}>
                    <div className="flex justify-between items-start mb-1.5">
                        <div className="flex-1 pr-2">
                            <h2 className="font-bold text-[15px] text-slate-900">{t.musteri}</h2>
                            <div className="flex items-center flex-wrap gap-1 mt-1.5">
                                <span className="text-[9px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded font-black border border-slate-200">📅 {aktifSekme==='teklifler' ? formatTarihKisa(t.is_tarihi) : `ONAY: ${formatTarihKisa(t.is_tarihi)}`}</span>
                                <span className="text-[9px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded font-black border border-blue-100">{t.is_modeli} • {t.adet || 1}</span>
                            </div>
                        </div>
                        <div className="flex flex-col items-end">
                            <div className="text-right text-[13px] font-black text-slate-900 mb-1">{t.fiyat}</div>
                            {zaman && !bittiMi && aktifSekme !== 'teklifler' && (
                                <div className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded-md border tracking-wider ${zaman.renkler.badge}`}>
          ...
          <div className="bg-white w-full max-w-sm p-6 rounded-[2rem] shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={isKaydet} className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                <select className="col-span-2 bg-slate-100 p-2.5 rounded-xl text-base font-bold border-none outline-none" value={yeniIs.isModeli} onChange={(e) => setYeniIs({...yeniIs, isModeli: e.target.value})}><option value="Cam Balkon">Cam Balkon</option><option value="Giyotin">Giyotin</option><option value="Sürme">Sürme</option><option value="Teras">Teras</option></select>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <select className="bg-slate-100 p-2.5 rounded-xl text-base font-bold border-none outline-none" value={yeniIs.kaynak} onChange={(e) => setYeniIs({...yeniIs, kaynak: e.target.value})}><option value="Instagram">Instagram</option><option value="Referans">Referans</option><option value="Web">Web</option></select>
              </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-100 px-6 py-2.5 flex justify-between items-center z-50 rounded-t-[1.5rem] shadow-2xl">
        <button onClick={() => setAktifSekme('teklifler')} className={`flex flex-col items-center gap-1 transition-all flex-1 ${aktifSekme === 'teklifler' ? 'text-blue-600' : 'text-slate-300'}`}><span className="text-[10px] font-black uppercase tracking-widest">TEKLİFLER</span>{aktifSekme === 'teklifler' && <div className="w-1.5 h-1.5 bg-blue-600 rounded-full"></div>}</button>
        <button onClick={() => setAktifSekme('siparisler')} className={`flex flex-col items-center gap-1 transition-all flex-1 ${aktifSekme === 'siparisler' ? 'text-blue-600' : 'text-slate-300'}`}><span className="text-[10px] font-black uppercase tracking-widest">SİPARİŞLER</span>{aktifSekme === 'siparisler' && <div className="w-1.5 h-1.5 bg-blue-600 rounded-full"></div>}</button>
        <button onClick={() => setAktifSekme('uretim')} className={`flex flex-col items-center gap-1 transition-all flex-1 ${aktifSekme === 'uretim' ? 'text-blue-600' : 'text-slate-300'}`}><span className="text-[10px] font-black uppercase tracking-widest">ÜRETİM</span>{aktifSekme === 'uretim' && <div className="w-1.5 h-1.5 bg-blue-600 rounded-full"></div>}</button>
      </div>
    </div>
  );
}